                    ...
                    children: [
                      ...
                      isVerified
                          ? buildButton('Sign In', () async {
                              String temp =
                                  await checkOTP(otpController.text.toString());
                              if (temp == "success") {
  ...
  sigInUsingPhoneNum(String phoneNumber) async {
    await auth
        ...
        .then((value) => {
              ...
              alertDialog(
                  'OTP sent', 'OTP has been sent to +91$phoneNumber', context),
            })
        .catchError(
            (error) => {alertDialog('Error', 'Try again Later', context)});
  }
    ...
    try {
      ...
      switch (e.code) {
        case "provider-already-linked":
          alertDialog(
              "Already Linked", 'The phone number is already linked', context);
          break;
        case "invalid-credential":
          alertDialog("Invalid Credential", 'Invalid Credential', context);
          break;
