...
class AddUserFunctions {
  ...
  addUser(String? phoneNumber, String companyName, String? mail, {required BuildContext context}) async {
    String uid = '';
    ...
    if (mail != '') {
      uid = await getUserByMail(mail!);
    } else if (phoneNumber != '') {
      uid = await getUserByPhone(phoneNumber!);
    }
    if (uid == "No user Found") {
      ...
      final newEmployeeRef =
          ref.child("companies/${companyName.capitalizeFirst}/members");//Database Path for Adding employee
      newEmployeeRef
          .update({
            uid: "employee",
          })
