...
getShipperIdFromCompanyDatabase() async {
  final snapshot = await ref
      .child(
          'companies/${shipperIdController.companyName.value.capitalizeFirst}/data')
      .get(); // This is the path for owner's shipper ID
  if (snapshot.exists) {
    String sid = snapshot.value
        .toString()
        .substring(6, snapshot.value.toString().length - 1);
    // the data we received will be in form of "{sid: shipper:abed1234..........}"
    shipperIdController.updateCompanyId(
        sid); // After getting the shipper id we are updating the shipper Id through out the app
  } else {
    debugPrint(
        'Error in get shipperId From company Database function'); // If there is no data exist then we are using the user's shipper id only
  }
}
